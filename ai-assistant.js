...
class AIVoiceAssistant {
        ...
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            this.wakeWordRecognition.onresult = (event) => {
                for (let i = event.resultIndex; i < event.results.length; i++) {
                    ...
                    // If already listening, process commands directly from wake word recognition
                    if (this.isWakeWordMode && event.results[i].isFinal) {
            ...
            console.warn('Web Speech API not supported. Using fallback button-only mode.');
            this.showToast('Voice commands require a modern browser with microphone support.');
        }
    }
    ...
    onWakeWordDetected() {
        if (!this.isListening && !this.isWakeWordMode) {
            ...
            // Reset wake word mode after a few seconds of inactivity
            if (this.wakeTimeout) {
...
ONLY respond with JSON, no other text.`;
        ...
        helpToast.innerHTML = `
            <div style="max-height: 300px; overflow-y: auto; text-align: left;">
                ...
                <div style="margin-top: 8px; font-size: 11px; color: var(--text-muted);">
                    Wake words: hello, hi, hey, aniquen
                </div>
            </div>
        `;
    ...
    start() {
        ...
        console.log('📋 Say "Aniquen", "Hello", "Hi", or "Hey" to wake me up!');
        console.log('📋 Or click the microphone button to start listening.');
        
        this.showToast('🎵 AI Assistant Ready! Say "Aniquen" or click the mic to start.');
    }
}
